...
fn ffi_export_contract_matches_declared_symbols() {
    assert_eq!(
        ...
        "update bindings/ffi/ffi-exports.txt for intentional desktop ABI changes"
    );
}
...
fn ffi_json_contract_snapshot_matches_declared_shapes() {
    ...
    if actual != expected {
        panic!(
            "FFI JSON contract changed; update bindings/ffi/ffi-json-contract.snapshot.json for intentional desktop API changes\n{}",
            serde_json::to_string_pretty(&actual).unwrap()
        );
    }
}
