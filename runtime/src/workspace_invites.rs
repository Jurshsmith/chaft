...
mod tests {
    ...
    #[test]
    fn rfc7748_invite_response_key_agreement_is_stable() {
        ...
        assert_zeroize_on_drop::<StaticSecret>();
        assert_zeroize_on_drop::<x25519_dalek::SharedSecret>();

        let alice_secret = StaticSecret::from(ALICE_PRIVATE_KEY);
        let bob_secret = StaticSecret::from(BOB_PRIVATE_KEY);
        let alice_public = X25519PublicKey::from(&alice_secret);
        let bob_public = X25519PublicKey::from(&bob_secret);

        assert_eq!(alice_secret.to_bytes(), ALICE_PRIVATE_KEY);
        assert_eq!(bob_secret.to_bytes(), BOB_PRIVATE_KEY);
        assert_eq!(alice_public.to_bytes(), ALICE_PUBLIC_KEY);
        assert_eq!(bob_public.to_bytes(), BOB_PUBLIC_KEY);

        let alice_shared = alice_secret.diffie_hellman(&bob_public);
        let bob_shared = bob_secret.diffie_hellman(&alice_public);
        assert_eq!(alice_shared.as_bytes(), &SHARED_SECRET);
        assert_eq!(bob_shared.as_bytes(), &SHARED_SECRET);
        assert_eq!(
            response_content_key(alice_shared.as_bytes()).as_bytes(),
            response_content_key(bob_shared.as_bytes()).as_bytes()
        );
    }
